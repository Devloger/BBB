								...
								else
								{
									...
									try
									{
										...
										else
										{
											...
											if(!($query = $connect->query("SELECT * FROM user WHERE id='$id'"))) {
												Throw new Exception($connect->error);
											}
											else
											{
												$data = $query->fetch_assoc();
												?><h1><?php echo $data['imie'];?> <?php echo $data['nazwisko'];?></h1>
												Avatar<br />
												...
												<br />
												<br />
												<a href="send.php?id=<?php echo $data['id'];?>">Wyślij Prywatną Wiadomość</a>
												<?php
											}
										}
									}
